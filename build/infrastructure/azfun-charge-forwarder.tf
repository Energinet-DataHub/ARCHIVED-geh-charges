...
module "azfun_charge_forwarder" {
  source                                         = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//function-app?ref=1.2.0"
  name                                           = "azfun-charge-forwarder-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name                            = data.azurerm_resource_group.main.name
  location                                       = data.azurerm_resource_group.main.location
  storage_account_access_key                     = module.azfun_charge_forwarder_stor.primary_access_key
  app_service_plan_id                            = module.azfun_charge_forwarder_plan.id
  storage_account_name                           = module.azfun_charge_forwarder_stor.name
...
module "azfun_charge_forwarder_plan" {
  source              = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//app-service-plan?ref=1.2.0"
  name                = "asp-charge-forwarder-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name = data.azurerm_resource_group.main.name
  location            = data.azurerm_resource_group.main.location
  kind                = "FunctionApp"
  sku                 = {
...
resource "random_string" "charge_forwarder" {
  length  = 6
  special = false
  upper   = false
}
