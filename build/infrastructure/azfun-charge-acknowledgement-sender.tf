...
module "azfun_charge_acknowledgement_sender" {
  source                                         = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//function-app?ref=1.2.0"
  name                                           = "azfun-charge-acknowledgement-sender-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name                            = data.azurerm_resource_group.main.name
  location                                       = data.azurerm_resource_group.main.location
  storage_account_access_key                     = module.azfun_charge_acknowledgement_sender_stor.primary_access_key
  app_service_plan_id                            = module.azfun_charge_acknowledgement_sender_plan.id
  storage_account_name                           = module.azfun_charge_acknowledgement_sender_stor.name
...
module "azfun_charge_acknowledgement_sender_plan" {
  source              = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//app-service-plan?ref=1.2.0"
  name                = "asp-charge-acknowledgement-sender-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name = data.azurerm_resource_group.main.name
  location            = data.azurerm_resource_group.main.location
  kind                = "FunctionApp"
  sku                 = {
...
module "azfun_charge_acknowledgement_sender_stor" {
  source                    = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//storage-account?ref=1.2.0"
  name                      = "stormsgrcvr${random_string.charge_acknowledgement_sender.result}"
  resource_group_name       = data.azurerm_resource_group.main.name
  location                  = data.azurerm_resource_group.main.location
...
resource "random_string" "charge_acknowledgement_sender" {
  length  = 6
  special = false
  upper   = false
}
