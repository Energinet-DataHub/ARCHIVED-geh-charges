...
module "azfun_charges_persist_changes" {
  source                                    = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//function-app?ref=1.0.0"
  name                                      = "azfun-persist-changes-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name                       = data.azurerm_resource_group.main.name
  location                                  = data.azurerm_resource_group.main.location
  storage_account_access_key                = module.azfun_charges_persist_changes_stor.primary_access_key
  app_service_plan_id                       = module.azfun_charges_persist_changes_plan.id
  storage_account_name                      = module.azfun_charges_persist_changes_stor.name
  ...
  app_settings                              = {
    ...
    LOCAL_EVENTS_SUBSCRIPTION_NAME               = azurerm_servicebus_subscription_rule.sbs-charge-business-validated-received-filter.subscription_name
  }
  dependencies                              = [
    module.appi.dependent_on,
    module.azfun_charges_persist_changes_plan.dependent_on,
    module.azfun_charges_persist_changes_stor.dependent_on,
...
module "azfun_charges_persist_changes_plan" {
  source              = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//app-service-plan?ref=1.0.0"
  name                = "asp-persist-changes-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name = data.azurerm_resource_group.main.name
  location            = data.azurerm_resource_group.main.location
  kind                = "FunctionApp"
  sku                 = {
...
resource "random_string" "charges_persist_changes" {
  length  = 6
  special = false
  upper   = false
}