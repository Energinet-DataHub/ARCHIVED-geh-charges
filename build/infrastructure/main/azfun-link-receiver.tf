...
module "azfun_link_receiver" {
  ...
  app_settings                              = {
    ...
    CHARGE_LINK_RECEIVED_TOPIC_NAME               = module.sbt_link_command_accepted.name
    CHARGE_LINK_RECEIVED_SENDER_CONNECTION_STRING = trimsuffix(module.sbtar_link_command_accepted_sender.primary_connection_string, ";EntityPath=${module.sbt_link_command_accepted.name}")
  }
  dependencies                              = [
