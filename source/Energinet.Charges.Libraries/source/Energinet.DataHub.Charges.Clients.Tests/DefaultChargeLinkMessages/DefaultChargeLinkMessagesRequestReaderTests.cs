...
{
    [UnitTest]
    public class DefaultChargeLinkMessagesRequestReaderTests
    {
        private string? _knownMeteringPointIdTestResult;

        [Theory]
        [InlineAutoMoqData("knownMeteringPointId1234")]
        public async Task ReadAsync_map_data_as_dto(string meteringPointId)
        {
            // Arrange
            ...
            var data = createDefaultChargeLinkMessages.ToByteArray();

            var target = new DefaultChargeLinkMessagesRequestReader(HandleDataAvailable);
        ...
        private async Task HandleDataAvailable(
            CreateDefaultChargeLinkMessagesDto createDefaultChargeLinkMessages)
        {
            _knownMeteringPointIdTestResult = createDefaultChargeLinkMessages.MeteringPointId;
