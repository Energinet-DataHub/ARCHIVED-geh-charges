...
namespace GreenEnergyHub.Charges.Application.Charges.Acknowledgement
{
    public class ChargePublisher : IChargePublisher
    {
        ...
        public async Task PublishChargeCreatedAsync(ChargeInformationDto chargeInformationDto)
        {
            var chargeCreatedEvent = _chargeCreatedEventFactory.Create(chargeInformationDto);
