...
namespace GreenEnergyHub.Charges.Tests.Domain.Charges
{
    ...
    public class ChargeTests
    {
        ...
        public void UpdateCharge_NewPeriodStartsInsideFirstOfThreeExistingPeriods_InsertsNewPeriod()
        {
            ...
            sut.Periods.GetValidChargePeriodAsOf(todayAtMidnightUtc)?.Name.Should().Be("NewPeriod");
            sut.Periods.GetValidChargePeriodAsOf(InstantHelper.GetTomorrowAtMidnightUtc())?.Name.Should().Be("NewPeriod");
            sut.Periods.GetValidChargePeriodAsOf(InstantHelper.GetTodayPlusDaysAtMidnightUtc(2))?.Name.Should().Be("NewPeriod");
            /*actualSecondPeriod.Name.Should().Be("NewPeriod");
            actualSecondPeriod.StartDateTime.Should().Be(todayAtMidnightUtc);*/
            // actualSecondPeriod.EndDateTime.Should().Be(InstantHelper.GetEndDefault());
        }
