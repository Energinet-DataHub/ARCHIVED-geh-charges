    ...
    public class ChargeCommandConverter : DocumentConverter, IChargeCommandConverter
    {
        ...
        private async Task<ParseSeriesPeriodResult> ParseSeriesPeriodIntoOperationAsync(SchemaValidatingReader reader, Instant startDateTime)
        {
            ...
            while (await reader.AdvanceAsync().ConfigureAwait(false))
            {
                ...
                {
                    var content = await reader.ReadValueAsDurationAsync().ConfigureAwait(false);
                    periodResolution = ResolutionMapper.Map(content);
                }
                ...
                {
                    var point = await ParsePointAsync(reader, periodResolution, startDateTime).ConfigureAwait(false);
                    points.Add(point);
                }
                else if (reader.Is(
            ...
            return new ParseSeriesPeriodResult(points, periodResolution, startDateTime, endDateTime);
        }
