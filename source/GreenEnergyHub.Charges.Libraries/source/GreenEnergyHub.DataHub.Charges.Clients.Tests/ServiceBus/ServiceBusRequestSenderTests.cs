...
namespace GreenEnergyHub.DataHub.Charges.Clients.CreateDefaultChargeLink.Tests.ServiceBus
{
    [UnitTest]
    public class ServiceBusRequestSenderTests
    {
        ...
        public async Task SendsMessage(
            ...
            string createLinkRequestQueueName,
            string replyToQueueName,
            string correlationId)
        {
            // Arrange
            var serviceBusSenderMock = new Mock<ServiceBusSender>();
            serviceBusClientMock.Setup(x => x.CreateSender(createLinkRequestQueueName))
